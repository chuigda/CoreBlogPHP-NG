...
使用最简洁的语言来探索语义概念，正是奥卡姆剃刀原理的一个例证 (#link("https://people.cs.nott.ac.uk/pszgmh/123.pdf#cite.occams-razor")[Duignan, 2018])。奥卡姆剃刀原理是一种哲学原则，它倾向于用最简单的解释来阐明某种现象。尽管由证书和加法组成的语言没有提供实际编程所需的特性，但它却提供了恰好足以解释许多语义学概念的结构。特别地，整数提供了一个简单的“值”的概念，而加法运算符则提供了一个简单的“计算”的概念。这一语言在过去曾被许多作者使用，例如 McCarthy 和 Painter (#link("https://people.cs.nott.ac.uk/pszgmh/123.pdf#cite.mccarthy")[1967]), Wand (#link("https://people.cs.nott.ac.uk/pszgmh/123.pdf#cite.wand")[1982]) 和 Wadler (#link("https://people.cs.nott.ac.uk/pszgmh/123.pdf#cite.expression-problem")[1998])，仅举几例。而本文开创了将这一语言作为通用工具，探索不同语义学话题的先例。
...
在文章的第一部分，我们展示了如何使用我们简单的表达式语言，来解释和比较多种不同的为语言赋予语义的方法。在本节中，我们将探讨语义学的指称方法 (#link("https://people.cs.nott.ac.uk/pszgmh/123.pdf#cite.strachey")[Scott 和 Strachey, 1971])——使用#term[值化函数 (valuation function)] 将语言中的#term[词项 (term)] 映射到适当#term[语义域 (semantic domain)] 中的#term[值 (value)] 来定义词项的#term[意义]。
...
组合性简化了推理过程，因为它允许我们进行“#term[以等换等 (replace equals by equals)]”的替换。例如，我们的表达式予以满足以下特性：
