...
#term[类型规则 (type rule)]大都形如 $Gamma tack e :: tau$，这表示在语境 $Gamma$ 中，词项 $e$ 的类型是 $tau$。#term[类型语境 (typing context)] 中列出了有效的基本类型，并将标识符与类型信息相关联。我们用 $alpha :: *$ 表示 $alpha$ 是一个基本类型，用 $x :: tau$ 表示词项 $x$ 的类型是 $tau$。

$
  Gamma ::= & epsilon           & #h(1em) & "空的类型语境" \
          | & Gamma, alpha :: * & #h(1em) & "添加一个类型标识符" \
          | & Gamma, x :: tau   & #h(1em) & "添加一个词项标识符"
$

词项和类型中的每个#term[自由变量 (free variable)] 都必须在类型语境中出现。例如，如果我们要声明 `const` 具有类型 $(beta -> beta) -> alpha -> beta -> beta$，则类型语境中至少应该包含：
...
基于上述规则，我们在图 2 中给出了类型语境及其有效性的定义。
...
$, caption: [#stlc()中的类型语境和#term[良构 (well-formed)] 的类型])

图 2 中的最后两条规则解释了一个类型在什么情况下是良构的——当类型中的所有自由变量都在语境中的时候。在类型#term[良构性 (well-formedness)] 规则以及后续的类型规则中，我们隐含地假设所有类型语境都有效。
...
最终，我们可以给出如图 3 所示的类型规则。我们不推断被 $lambda$ 绑定的变量的类型。也就是说总体上我们只进行类型检查。不过我们可以很容易地判定带类型注解的词项、变量和应用的类型。因此，当一个类型是类型检查算法的输入时，我们给类型规则标上 #tin；当类型是类型检查算法的输出时，我们给类型规则标上 #tout。目前这些记号只是为了提供一些直觉，但在实现中，两种规则之间的差异会变得明显。
...
在着手实现类型检查之前，我们须先定义类型语境。语境以（反向的）列表形式实现，将名称#linebreak()与 $*$ (`HasKind Star`) 或者一个类型（`HasType τ`）关联起来。语境的扩展自然是通过列表的 `cons` 操作 (`:`) 实现的；在语境中查找名称则是使用 Haskell 的标准列表函数 `lookup` 进行的。
...
类型检查函数接受一个整数，该整数表示我们进入了多少层绑定符。初次调用时，这个参数应该为 `0`，我们为此提供了包装函数 `type↑0`。我们用这个整数模拟处理绑定变量时的类型规则：在 $lambda$ 抽象的类型规则 [LAM] 中，在检查函数体时，我们将绑定变量添加到语境中——而我们的实现正是这么做的。计数器 `i` 表示我们进入了多少层绑定符，因此 `Local i` 总是一个可用的新名称。我们先将 `Local i` 添加到语境 `Γ` 中，将其与绑定变量关联起来，然后对函数体作类型检查。因为我们把一个绑定变量变成了一个自由变量，所以我们要对函数体进行相应的替换。类型检查器不会遇到绑定变量，因此函数 `type↑` 没有用于处理 `Bound` 的分支。
...
在接触类型规则之前，我们得先回顾一下类型语境。因为现在一切皆词项，语境的抽象语法和有效性规则（图 9，与图 2 对比）自然也相应地简化了。
...
#align(center)[图 9#h(1em)#dtlc()的类型语境及其有效性]

语境中现在只有一种形式的条目，也就是说我们总是假设变量有其类型。注意我们在语境中存储的都是求值后的类型。非空语境有效性规则中的前提条件 $Gamma tack tau tin *$ 指涉的不再是一个特殊的类型良构性的判断，而是我们即将定义的类型规则——我们不再需要为类型设置专门的良构性规则了。这一前提条件尤其确保了 $tau$ 中不含有未知的变量。

类型规则在图 10 中给出。类型规则现在与语境、词项和#term[值]相关——所有类型都被尽可能早地求值。和之前一样，我们高亮了规则中和图 3 不同的部分。我们仍然为推断和检查使用不同的符号：当一个类型是输出时，我们用 $tout$；当一个类型是输入时，我们用 $tin$。新的构造 $*$ 和 $forall$ 属于我们能推断其类型的那一类。和之前讨论#stlc()的时候一样，我们假定类型规则中出现的所有语境都是有效的。
...
依值函数空间的规则 [PI] 和图 2 中箭头类型的良构性规则 [FUN] 有相似之处。依值函数的输入类型 $rho$ 和返回类型 $rho'$ 都必须归属于#term[种类] $*$。与之前的规则 [FUN] 有所不同的是，$rho'$ 中可以包含#footnote[译注：“包含”原作“指涉”。] $x$，因此我们在检查 $rho'$ 的时候要将 $x :: tau$（其中 $tau$ 是对 $rho$#footnote[译注："$rho$" 原作 "$tau'$"，但规则 [PI] 中并无 $tau'$，故依实际情况改写为 $rho$。] 求值的结果）加入语境 $Gamma$。
...
要适配求值器，我们只需为函数 `eval↑` 添加两个用于处理 `Star` 和 `Pi` 的新分支，如图 11 所示（#stlc()的求值器见图 4）。`Star` 的求值非常简单。对于 `Pi`，我们求值其输入类型和返回类型，且在求值返回类型时，我们需要将绑定变量 $x$ 添加到语境中。
...
语境将变量映射到其类型，而类型现在也在词项层级上。我们存储的是类型求值后的形式，因此我们如是定义类型和语境：
...
对于依值函数类型 $forall x :: rho thin . rho'$，我们首先对输入类型 $rho$ 作#term[种类]检查，然后将其求值为 $tau$#footnote[译注：原文为 "For a dependent function type, we first kind-check the domain $tau$. Then the domain is evaluated to $v$." 此处显然为作者笔误。]。接着，我们将值 $tau$ 加入语境，对返回类型 $rho'$ 作#term[种类]检查——这里的思路跟#stlc()和#dtlc()中对 `Lam` 作类型检查的规则有异曲同工之妙。
...
对于 $lambda$ 抽象 $lambda x -> e$，现在我们要求其具有依值函数类型 `VPi τ τ'`。和#stlc()一样，我们要在对函数体 $e$ 作类型检查时将绑定变量 $x$（类型为 $tau$）加入语境；但与#stlc()不同的是，现在除了要用 `subst↓` 替换函数体 $e$ 中的 $x$ 之外，还要将 `τ'` 应用于 `(Local i)` 来替换 $tau'$ 中的 $x$。
