...
最后，我们要重申我们现在所展示的类型系统是#term[不健全 (unsound)] 的。因为 $*$ 本身的#term[种类]也是 $*$，我们可以编码出#term[罗素悖论 (Russell's paradox)] 的某种变体——#term[吉拉德悖论 (Girard's paradox)] [3]。这使得我们可以创造出一个具有任何类型的#term[居留元 (inhabitant)]#footnote[译注：在类型论中，如果存在一个词项 $t$ 使得 $t :: τ$，则称 $t$ 是类型 $τ$ 的一个#term[居留元]。根据柯里-霍华德同构，类型对应命题，而居留元对应命题的证明，能构造出具有任何类型的居留元也就意味着能“证明任何命题”。]。要修复这个问题，标准的解决方案是为类型引入一个无限的层级：$*$ 的类型是 $*_1$，$*_1$ 的类型是 $*_2$，依此类推。
