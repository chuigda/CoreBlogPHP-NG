...
只有将数据类型添加到这个基础演算中，才能真正发挥依值类型的全部威力。因此，我们在第 4 节中演示了如何使用自然数和#term[向量 (vector)]扩展我们的语言。使用这些新增的数据类型，我们编写了经典的向量追加操作，以演示如何在我们的核心演算中进行编程。而利用本节解释的原理，可以向演算中加入更多数据类型。
...
$
  e ::= & e :: tau      & #h(2em) & "带类型注解的词项" ^#footnote[类型学家使用符号 "$:$" 或者 "$in$" 表示类型归属关系。然而，在 Haskell 中，符号 "`:`" 被用作列表的 `cons` 运算符，因此 Haskell 的设计者为类型注解选择了非标准的"$::$"。本文将尽可能地遵循 Haskell 语法，以缩小本文所涉及的不同语言之间的语法差距。] \
      | & x             & #h(2em) & "变量" \
      | & e thin e'     & #h(2em) & "应用" \
...
另一种做法是，我们可以要求抽象语法中的每个 Lambda 绑定变量都必须显式标注类型——这样我们就只拥有可推断项了。然而，能够为任意词项标注类型是非常有用的。既然有了通用的标注机制，就不再需要强制要求每个 Lambda 绑定变量都带类型注解了。事实上，允许未标注的 Lambda 在不增加额外成本的情况下提供了很大的便利：形如 $e thin (lambda x -> e')$ 的函数应用可以在没有类型标注的情况下被处理，因为 $x$ 的类型是由 $e$ 的类型决定的。
...
我们的简单类型 Lambda 演算求值器就快完成了。目前还剩一个小问题：求值器返回的是 `Value`，而当下我们无法打印 `Value` 类型的值，也不能对 `Value` 作相等性判断。这是因为 `Value` 类型的 `VLam` 构造子接受的是一个 Haskell 函数，我们不能像对其他类型那样简单地为其派生 `Show` 和 `Eq`#footnote[译注：此处译文结构较原文有较大变化。]。因此，如果我们要重新得到一个值的内部结构，我们就需要 `quote` 函数。代码在图 7 中给出。
...
最后一条规则 [LAM] 是用来检查 Lambda 抽象的。与之前不同的是，Lambda 抽象的类型现在是一个依值函数类型了，约束变量 $x$ 不仅可能出现在函数体 $e$ 中，还可能出现在返回类型 $tau'$ 中#footnote[译注：后半句“还可能出现在返回类型 $tau'$ 中”为译者补文。]。因此在对函数体 $e$ 作类型检查和对返回类型 $tau'$ 作#term[种类]检查时，都要使用扩展过的上下文 $Gamma, x :: tau$。

总结下来，我们所作的所有修改都是围绕着我们在第 3 节开头引入的两个核心概念进行的：函数空间被泛化为依值函数空间；类型和#term[种类]也是词项。
...
和之前一样，我们用高阶抽象语法表示#term[值]，也就是用 Haskell 函数来表示约束结构#footnote[译注：也就是 $lambda$ 和 $forall$/$Pi$。]。我们用 `VPi` 表示新的约束结构 $forall$/$Pi$。在依值函数空间中，由 $forall x : A tdt B$ 引入的约束变量 $x$ 不会出现在变量自身的类型 $A$ 中，但函数的返回类型 $B$ 中却有可能包含 $x$#footnote[译注：此句为译者改写。原文为 "In the dependent function space, a variable is bound that is visible in the range, but not in the domain."]。因此，输入类型可以简单地用一个 `Value` 表示，而返回类型则要用 Haskell 函数 `Value -> Value` 表示#footnote[译注：不妨把 $Pi x : A tdt B$ 看作 $Pi x : A tdt B(x)$，即类型 $B$ 是关于值 $x$ 的函数。]。
