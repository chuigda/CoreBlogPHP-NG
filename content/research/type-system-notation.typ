...
更复杂的语言自然会有更复杂的文法：命令式语言需要#term[语句 (statement)] 的文法，支持模式匹配的语言需要模式的文法，诸如此类。而上面这个简单语言里甚至连变量都没有！然而，在文法上区分#term[词项 (term, 具有类型的东西)] 和类型是必要的，因为类型系统正是要定义词项和类型间的关系。

#colbreak()

== 关系、判断、公理和推理规则

指定文法之后，下一步就是定义#term[类型关系 (typing relation)]。类型关系通常写作 $e : tau$，可以读作“$e$ 的类型是 $tau$”。直觉上，一些陈述是“有意义”的，另一些则没有：

- $1 + 2 : "Int"$ 表示“$1 + 2$ 的类型是 Int”，这当然是有意义的。
- $1 + 2 : "Bool"$ 表示“$1 + 2$ 的类型是 Bool”，而这是没有意义的。
- $"true" + 2 : "Int"$ 表示“$"true" + 2$ 的类型是 Int”，这更没有意义，因为 $"true" + 2$ 本就是无稽之谈，根本不该有类型。

我们应该写一些能准确捕捉“哪些陈述有意义，哪些没有”直觉的规则。为此，我们定义#term[类型判断 (typing judgement)]，写作：
...
用这种符号，我们就可以为类型系统写一些#term[类型规则 (typing rule)]了：
...
这两条规则上面都有一条横线，而横线上空无一物，这表示它们恒成立，也就是说它们是#term[公理 (axiom)]。对于整数字面量，也可以有无数这样的公理：
...
应用这条规则时可以为 $tau$ 选取任何类型，只要它在两个分支之间保持一致就行。
...
幸运的是，很多时候，你都可以用一种方法照着类型规则写出类型检查算法：规则 $tack e : tau$ 可以解释成一个从表达式 $e$ 到其类型 $tau$ 的#term[函数 (function)]。通常表达式文法中的每种情况都有相应的一条规则，这样整个类型规则就能转写成一个递归的类型检查函数，每条规则对应于这个递归函数中的一条分支。
...
目前为止，我们用作例子的语言异常地简单。此前我一直在有意规避#term[变量 (variable)] 这一复杂性，但若要为任何有用的程序设计语言编写类型规则，那么就不能逃避。所以接下来让我们扩展这个小小语言，向其中加入函数，使其成为#link("https://en.wikipedia.org/wiki/Simply_typed_lambda_calculus")[#term[简单类型 $lambda$ 演算 (Simply-typed lambda calculus, STLC)]] 的一个变体。这需要向语言的文法中添加如下内容：
...
问题在于，变量的类型取决于它出现时所处的#term[语境 (context)]。因此类型判断需要相应地扩展，以追踪作用域内所有变量的类型，我们用使用以下符号：
...
通常，推理规则横线上面出现的条件必须是能通过公理和推理规则的某种组合来满足的判断。然而，事情并不总是如此：规则中也可能包含任意的布尔表达式，我们称之为#term[附加条件 (side conditions)]，只有所有附加条件都满足，才能应用推理规则。例如在上述类型规则中，$x : tau in Gamma$ 就是一个附加条件。
