...
去函数化 #link("(Reynolds, 1972)") 是消除高阶函数参数的标准技术。这一技术基于以下观察：我们不需要支持任意的高阶函数参数，因为被用作实参的高阶函数只有寥寥几种。因此，我们可以用纯数据类型代替函数类型，来表示我们实际所需的高阶函数参数。
...
在文献中这一函数常被称作 `apply` #link("(Reynolds, 1972)")：若将 `exec` 的类型展开成 #linebreak() `CONT -> Integer -> Integer`，则这一函数可被视作将续延的表示应用于一个整数，以得到另一个整数。而本文选用 `exec` 的缘由稍后便会揭晓。
